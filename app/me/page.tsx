...
import { User, ArrowLeft, Clock, MapPin } from "lucide-react"
...
export default function MePage() {
  ...
  // For now, we'll just use the first two entries as examples
  // Later we'll implement proper user tracking
  const allEntries = getAllEntries().slice(0, 2)
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-100 dark:from-gray-900 dark:to-gray-800">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">My Entries</h1>
            </div>
            <Link href="/public">
              <Button variant="outline" size="sm" className="gap-2">
      ...
      <div className="max-w-2xl mx-auto p-4">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Your Personal Journal</h2>
          <p className="text-gray-600 dark:text-gray-400">
            {allEntries.length === 0
              ? "Start writing to see your entries here"
              : `${allEntries.length} ${allEntries.length === 1 ? "entry" : "entries"} in your personal collection`}
          </p>
        </div>
          ...
          {allEntries.length === 0 ? (
            <Card className="text-center py-12">
              <CardContent>
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-4">
                  Start writing to build your personal collection of thoughts and feelings
                </p>
                <Link href="/">
                  <Button className="bg-green-600 hover:bg-green-700 text-white">Write First Entry</Button>
                </Link>
              </CardContent>
            </Card>
          ) : entries.length === 0 ? (
            <Card className="text-center py-12">
              <CardContent>
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-4">You haven't written any {selectedMood} entries</p>
                <div className="flex gap-2 justify-center">
                  <Link href="/">
                    <Button className="bg-green-600 hover:bg-green-700 text-white">Write Entry</Button>
                  </Link>
        ...
        {allEntries.length > 0 && (
          <div className="text-center mt-8">
            <Link href="/">
              <Button className="bg-green-600 hover:bg-green-700 text-white">Write Another Entry</Button>
            </Link>
          </div>
        )}
      </div>
